...
const youtube = require('./Youtube/youtube-live');

welcome(client)

// --- Startup ---------------------------------------------------------------------------------------------------------
console.log('DALENG_STREAM_COMMUNITY_ is starting.');
try {
    //code
    } catch (error) {
...
client.on("message", (message) => {
    if (message.content !== "/rverify") return;
      ...
      .setTitle("Velkommen til \*DALENG_STREAM_COMMUNITY_\*")
      .setDescription("Trykk på :white_check_mark: under for å bekrefte at du har lest slik at du får tilgang til resten av serveren!")
      .setColor('#38A800')
    ...
    client.on('clickButton', async (button) => {
        ...
        button.reply.send('Du er nå verifisert!', true)
        const role = button.guild.roles.cache.get("921071387966328862") 
...
TwitchMonitor.onChannelLiveUpdate((streamData) => {
    ...
    // Generate message
    const msgFormatted = `@everyone ${streamData.user_name} Er nå live. Gjerne sjekk ut hva han holder på med. https://www.twitch.tv/${streamData.user_login}`;
